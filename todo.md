...
- [ ] Integration tests: at least one benchmark instance solved end-to-end per relevant crate — MIPLIB 2017 (MILP), MINLPLib (MINLP), Netlib (LP/network), CSPLib (CP) — see per-phase integration test items above; MILP coverage now spans p0033 in-tree plus the fetched-corpus suite (`benchmark_corpus.rs`: afiro/adlittle/e226 LP, flugpl/gt2/egout MILP, bell5 soundness); full-corpus runners for MINLPLib/Netlib/CSPLib remain future work
...
- [x] **`tpt-opt-core` `serde` feature did not compile (build regression)**: the `serde` workspace dependency was pinned with `default-features = false` + `features = ["derive"]` only, so the derived `Serialize`/`Deserialize` impls on core types referenced `Vec`/`String` (and other `alloc` types) without serde's `alloc` feature, failing `cargo check --all-features` with 58 errors (`the trait bound Vec<…>: serde::Serialize is not satisfied`). **FIXED (2026-08-23)**: enabled serde's `alloc` feature workspace-wide (`features = ["derive", "alloc"]`) and made `tpt-opt-core`'s `serde` feature require `alloc`; the feature now compiles and round-trips model / solution / parameters / bounds / errors through JSON (`tests/serde_roundtrip.rs`, 7 passing cases). Also cleared two latent clippy warnings surfaced by `--all-targets`: a `neg_cmp_op_on_partial_ord` in `tpt-opt-cli` (`--time-limit` validation) and `useless_format` + `items_after_test_module` + missing `Default` in `tpt-opt-cp::model`; the workspace is now clippy-clean (excluding the non-default `highs` feature, which needs a C++ toolchain unavailable in this environment).
...
- [x] `tpt-opt-multi`: weighted-Tchebycheff scalarisation with adaptive weights — implemented as `LinearMultiObjective::solve_weighted_tchebycheff_adaptive` (+ `solve_adaptive_tchebycheff` convenience wrapper): repeated augmented-Tchebycheff solves whose normalised deviations rebalance weights toward a balanced achievement, with best-achievement tracking across iterates; deterministic (no randomness); tested for skewed-extreme escape, same-seed determinism, and η=0 reduction to plain augmented Tchebycheff
...
- [x] `tpt-opt-network`: audit `graph_preprocess.rs` for series-parallel reduction â€” audit complete (2026-08-23): `series_parallel_check`/`SeriesParallelReport` is present, exported from the crate root, and unit-tested (series contraction, parallel-edge merge, dangling prune, K4 negative, bowtie/two-triangle positives); the Phase 3 flag was stale
...
- [ ] Real benchmark-corpus integration tests (full MIPLIB 2017 / MINLPLib / Netlib / CSPLib runs, not just one hand-picked instance per crate) â€” first slice landed: `crates/tpt-opt-milp/tests/benchmark_corpus.rs` solves curated Netlib LP classics (afiro/adlittle/e226) and MIPLIB MILP classics (flugpl/gt2/egout + bell5 soundness) against published optima, fetched out-of-band via `cargo xtask fetch-fixtures`; expanding to the full corpora per suite remains future work
...
- [x] `xtask`/CI step to fetch and cache MIPLIB/MINLPLib/Netlib/CSPLib fixtures outside the packaged crate (`tests/fixtures/`, excluded via `Cargo.toml` `exclude`) — implemented as `cargo xtask fetch-fixtures` (lists/force/per-suite subset support); wired into CI as the non-blocking `benchmark-corpus` job that fetches then runs `tests/benchmark_corpus.rs` against the published optima
- [ ] Python bindings via PyO3 for at least `tpt-opt-milp`/`tpt-opt-heuristic`/`tpt-opt-multi`
- [x] Solver progress-callback API (iteration count, bound, incumbent) so callers can drive progress bars / early-termination policies — `tpt-opt-core::progress` (`ProgressEvent`/`ProgressAction`) + `tpt-opt-milp::MilpSolver::with_progress_callback`; `tests/progress_callback.rs` exercises abort-on-bound and live incumbent capture
- [x] Small CLI (`tpt-opt-cli`) that reads an MPS/LP file and prints a solution — implemented (`src/main.rs`): MPS + CPLEX-LP readers, `--quiet`/`--cuts`/`--time-limit`/`--threads` flags; per-crate README/CHANGELOG present
- [x] `serde`-gated model/solution serialization (warm-start caching across runs, reproducible bug reports) — `tpt-opt-core` derives `Serialize`/`Deserialize` on `Model`/`Solution`/`SolveParameters`/`Tolerances`/`WarmStart`/`OptimizationError`/`VarBound`/`BoundKind` behind a `serde` feature; `tests/serde_roundtrip.rs` (7 cases) passes; JSON cannot represent ±inf bounds (documented in `bounds.rs`)
...
- [x] `cargo-generate` template (or template repo) for a new Tier 2 consumer crate, scaffolding the `tpt-opt-systems` feature-subset dependency from the README's consumption table — `template/` dir present and referenced from the root README ("New Tier 2 consumer crates can be scaffolded from the bundled template")
- [x] `xtask new-crate <name>` recipe to scaffold a new `tpt-opt-*` crate from the 13-step "Per-Crate Checklist Template" (`Cargo.toml`, `lib.rs`, `README.md`, `CHANGELOG.md`, `LICENSE-*`) — implemented and clippy-clean (scaffolds crate dir, `Cargo.toml` inheriting workspace fields, `lib.rs` stub, `README.md`/`CHANGELOG.md`, copied `LICENSE-*`)
- [x] Root README badges (CI status, crates.io version once published, docs.rs, license) — present at the top of `README.md` (CI / license / MSRV / crates.io / docs.rs)
